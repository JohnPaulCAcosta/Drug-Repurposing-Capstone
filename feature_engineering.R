...
all.drugs = read.csv(here("drug_data", "literallyAllDrugs.csv"),
                     check.names = FALSE) %>%
  filter(
    !is.na(SMILES), 
    !is.na(Target), 
...
nP$IndicationVector = strsplit(nP$Indication, ", ")

nP$IndicationPrefixVector = lapply(nP$IndicationVector, function(vec) {
  unique(sapply(vec, function(x) {
    sub("^([A-Za-z]+).*", "\\1", x)
  }))
}
...
for (vecT in nP$IndicationPrefixVector) {
